...
#M:    dsetClassId = f.create_dataset('ClassId', (0,1), maxshape=(None,1), dtype=h5py.special_dtype(vlen=unicode))
    ...
    for i in range(len(fileRanges)-1):
        ...
        X, y, fn, cIds = spectrogramListToT4(tempSG,                             binaryLabels[fileRanges[i]: fileRanges[i+1]],                             filenames = df_mc.MediaId[fileRanges[i]: fileRanges[i+1]].values, N=spectrogramWindowLength,                             classIds = df_mc.ClassId[fileRanges[i]: fileRanges[i+1]].values) #convert to t4
...
uniqueClassNumber
print('uniqueClassNumber= ',uniqueClassNumber)
